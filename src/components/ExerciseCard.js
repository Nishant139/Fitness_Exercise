...
const ExerciseCard = ({exercise}) => {
  return (
    // each and every card going to link 
    // in `` theres is dynamic array
    <Link className='exercise-card' to={`/exercise/${exercise.id}`}>
        <img src={exercise.gifUrl} alt={exercise.name} loading='lazy'/>
        <Stack direction = "row" >
          <Button sx={{ ml:'21px' , color:'#fff' , background:'#ffa9a9' , fontSize:'14px' , 
          borderRadius:'20px' , textTransform:'capitalize' }}>
               {exercise.bodyPart}    {/*THIS FOR TEXT WHICH WRITE UNDER GIF */}
          </Button>

          <Button sx={{ ml:'21px' , color:'#fff' , background:'#fcc757' , fontSize:'14px' , 
          borderRadius:'20px' , textTransform:'capitalize' }}>
               {exercise.target}    {/*THIS FOR TEXT WHICH WRITE UNDER GIF */}
          </Button>
        </Stack>
